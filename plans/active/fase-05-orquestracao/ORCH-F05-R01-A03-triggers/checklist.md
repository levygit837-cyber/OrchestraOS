...
- [ ] 6. Criar `internal/modules/trigger/service.go` (Create, EvaluateRun, EvaluateSession, Resolve, Dismiss)
- [ ] 7. Criar `internal/modules/trigger/detectors.go` (Stall, Loop, Drift, PathViolation, Token, Steps, Time)
- [ ] 8. Criar `internal/modules/trigger/thresholds.go` + `validation.go` + `events.go` + `models.go`
- [ ] 9. Criar `contracts/schemas/trigger.schema.json`
- [ ] 10. Criar testes unitários para cada detector (casos positivos e negativos)
- [ ] 11. Criar testes de integração `internal/modules/trigger/service_test.go` (EvaluateRun, ListActive)
- [ ] 12. Rodar `go test ./...` — verificar regressão em todos os pacotes
- [ ] 13. Code review auto-crítico (determinismo, falsos positivos, memory leaks)
