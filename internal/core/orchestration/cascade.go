...
func CancelTaskDependents(ctx context.Context, tx *sql.Tx, taskID string, input transition.TransitionInput) error {
	...
	if err != nil {
		return apperrors.Wrap(apperrors.CodePersistence, "orchestration.cancel_task_dependents.list_runs", err)
	}
	...
	if err != nil {
		return apperrors.Wrap(apperrors.CodePersistence, "orchestration.cancel_task_dependents.list_work_units", err)
	}
	for _, wu := range workUnits {
		...
		if err != nil {
			return apperrors.Wrap(apperrors.CodePersistence, "orchestration.cancel_task_dependents.update_work_unit", err)
		}
		if err := dbcore.EnsureRowsAffected(res, "work unit", "orchestration.cancel_task_dependents.update_work_unit"); err != nil {
			return err
		}
	}
	return nil
