...
func (s *AgentSessionService) SuggestCheckpoint(ctx context.Context, sessionID string, input AutoCheckpointInput) (*CheckpointSuggestion, error) {
	...
	tx, err := beginTx(ctx, s.db, "agent_session_service.begin_suggest_checkpoint")
	if err != nil {
		return nil, err
	}
	defer rollbackTx(tx)
	if _, err := getAgentSession(ctx, tx, sessionID); err != nil {
		return nil, err
	}
	...
	if err := commitTx(tx, "agent_session_service.commit_suggest_checkpoint"); err != nil {
		return nil, err
	}
...
func (s *AgentSessionService) AutomaticCheckpoint(ctx context.Context, sessionID string, input AutoCheckpointInput) (*OperationResult[*domain.AgentSession], *CheckpointSuggestion, error) {
...
func (s *AgentSessionService) CheckpointFromEvent(ctx context.Context, sessionID string, event *domain.EventEnvelope) (*OperationResult[*domain.AgentSession], error) {
...
func (s *AgentSessionService) ListCheckpoints(ctx context.Context, sessionID string) ([]CheckpointRecord, error) {
	...
	tx, err := beginTx(ctx, s.db, "agent_session_service.begin_list_checkpoints")
	if err != nil {
		return nil, err
	}
	defer rollbackTx(tx)
	...
	events, err := NewEventServiceWithExecutor(tx).ListByRun(ctx, run.ID)
	if err != nil {
		return nil, err
	}
	...
	if err := commitTx(tx, "agent_session_service.commit_list_checkpoints"); err != nil {
		return nil, err
	}
	return records, nil
}
...
func (s *AgentSessionService) RecoverableCheckpoint(ctx context.Context, sessionID string) (*RecoverableCheckpointState, error) {
	...
	tx, err := beginTx(ctx, s.db, "agent_session_service.begin_recoverable_checkpoint")
	if err != nil {
		return nil, err
	}
	defer rollbackTx(tx)
	...
	if err := commitTx(tx, "agent_session_service.commit_recoverable_checkpoint"); err != nil {
		return nil, err
	}
