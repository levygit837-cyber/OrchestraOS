...
|--------|------|------------------|------------|
| agent | `internal/modules/agent` | Agent entities, Runtimes (Fake, Gemini, Codex), GeminiPlanner | core/apperrors, core/orchestration |
| agentsession | `internal/modules/agentsession` | Sessões de agente, checkpoints, heartbeat, timeout | core/apperrors, core/db, core/orchestration, core/statemachine |
| orchestrator | `internal/modules/orchestrator` | OrchestratorService - coordena fluxo end-to-end de tasks | core/apperrors, core/db, core/orchestration |
| prompt | `internal/modules/prompt` | Prompt fragments, snapshots, toolsets | core/apperrors, core/db |
| review | `internal/modules/review` | Gates de revisão e validação | core/apperrors, core/db, core/orchestration |
| run | `internal/modules/run` | Execuções de agente, retry, projeção | core/apperrors, core/db, core/orchestration |
| task | `internal/modules/task` | Ciclo de vida de tarefas | core/apperrors, core/db, core/orchestration |
...
| workunit | `internal/modules/workunit` | Work units, dependências, paths | core/apperrors, core/db, core/orchestration |
...
| orchestration | `internal/core/orchestration` | Cross-domain: TransitionInput, OperationResult, RuntimeEventRelay, PromptOrchestrator |
...
- Comunicação cross-module ocorre via `internal/core/orchestration/` ou interfaces DI com adapters em `internal/bootstrap/services.go`
