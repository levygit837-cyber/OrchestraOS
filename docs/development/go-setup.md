...
```sh
./scripts/go/install-go-local.sh
```

Por padrao o script instala `go1.26.2` em `.tools/go`. Para usar outra versao:

```sh
GO_VERSION=1.26.2 ./scripts/go/install-go-local.sh
```

Depois disso, `./scripts/go/go` usa automaticamente `.tools/go/bin/go`.
...
```sh
./scripts/go/go env GOROOT GOPATH GOMODCACHE GOCACHE GOTOOLCHAIN
```

Se `go list ./...` falhar com erro de codigo, trate o erro no pacote indicado. Se falhar tentando escrever em `/home/.../go` ou `/home/.../.cache`, use `./scripts/go/go` em vez do binario global diretamente.
...
./scripts/go/go run ./cmd/orchestraos --db-port 55432 migrate up
TEST_DB_DSN="host=localhost port=55432 user=orchestraos password=orchestraos dbname=orchestraos sslmode=disable" ./scripts/go/go test ./...
```
